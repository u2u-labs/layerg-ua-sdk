...
contract GAccountExtension is ContractMetadata, ERC1271, GAccountPermissions, ERC721Holder, ERC1155Holder {
    ...
    modifier onlyAdminOrEntrypoint() virtual {
        require(
            msg.sender == address(GAccountCore(payable(address(this))).entryPoint()) || isAdmin(msg.sender),
    ...
    function addDeposit() public payable {
        GAccountCore(payable(address(this))).entryPoint().depositTo{ value: msg.value }(address(this));
    }

    /// @notice Withdraw funds for this account from Entrypoint.
    function withdrawDepositTo(address payable withdrawAddress, uint256 amount) public {
        _onlyAdmin();
        GAccountCore(payable(address(this))).entryPoint().withdrawTo(withdrawAddress, amount);
    }
    ...
    function _registerOnFactory() internal virtual {
        address factory = GAccountCore(payable(address(this))).factory();
        GBaseAccountFactory factoryContract = GBaseAccountFactory(factory);
        if (!factoryContract.isRegistered(address(this))) {
            factoryContract.onRegister(GAccountCoreStorage.data().creationSalt);
        }
    }
