...
import "./interfaces/IGAccountCore.sol";
import "./upgradeable/Initializable.sol";
import "../extension/Multicall.sol";
import "./GAccountPermissions.sol";
import "../core/BaseAccount.sol";
import "../interfaces/IEntryPoint.sol";
import "./GAccountCoreStorage.sol";
import "./GAccountExtension.sol";
...
contract GAccountCore is IGAccountCore, Initializable, Multicall, GAccountPermissions, BaseAccount {
    ...
    function initialize(address _defaultAdmin, bytes calldata _data) public virtual initializer {
        // This is passed as data in the `_registerOnFactory()` call in `GAccountExtension` / `Account`.
        GAccountCoreStorage.data().creationSalt = _generateSalt(_defaultAdmin, _data);
    ...
    function entryPoint() public view virtual override returns (IEntryPoint) {
        address entrypointOverride = GAccountCoreStorage.data().entrypointOverride;
    ...
    */
    ...
    function isValidSigner(address _signer, PackedUserOperation calldata _userOp) public view virtual returns (bool) {
        ...
        if (sig == GAccountExtension.execute.selector) {
    ...
    function setEntrypointOverride(IEntryPoint _entrypointOverride) public virtual {
        _onlyAdmin();
        GAccountCoreStorage.data().entrypointOverride = address(_entrypointOverride);
    }
    ...
    function _setAdmin(address _account, bool _isAdmin) internal virtual override {
        ...
        if (factory.code.length > 0) {
            if (_isAdmin) {
                GBaseAccountFactory(factory).onSignerAdded(_account, GAccountCoreStorage.data().creationSalt);
            } else {
                GBaseAccountFactory(factory).onSignerRemoved(_account, GAccountCoreStorage.data().creationSalt);
            }
        }
    }
    ...
    function _afterSignerPermissionsUpdate(SignerPermissionRequest calldata _req) internal virtual override {
        if (factory.code.length > 0) {
            GBaseAccountFactory(factory).onSignerAdded(_req.signer, GAccountCoreStorage.data().creationSalt);
        }
    }
