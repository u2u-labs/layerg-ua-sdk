...
import { PreVerificationGasCalculator, PreVerificationGasCalculatorConfig } from '@layerg-ua-sdk/aa-sdk'
import { AddressZero, CodeHashGetter__factory, decodeErrorReason, decodeRevertReason, EIP7702Authorization, EntryPointSimulationsJson, getAddr, getEip7702AuthorizationSigner, IEntryPoint, IEntryPoint__factory, IEntryPointSimulations__factory, IPaymaster__factory, mergeValidationDataValues, OperationBase, packUserOp, ReferencedCodeHashes, requireAddressAndFields, requireCond, RpcError, runContractScript, SenderCreator__factory, StakeInfo, StakeInfoStructOutput, StorageMap, UserOperation, ValidationErrors, ValidationResultStructOutput } from '@layerg-ua-sdk/aa-utils'
      ...
      if (call.topLevelTargetAddress == null && call.to.toLowerCase() === this.entryPoint.address.toLowerCase()) {
        if (call.input.length <= 2) {
          ...
          const mergedAbi = Object.values([
            ...
            const key = `(${entry.inputs.map(i => i.type).join(',')})`
            return {
              ...set,
              [key]: entry
            }
